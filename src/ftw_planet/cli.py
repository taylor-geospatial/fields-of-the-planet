...
from ftw_planet.eval import evaluate_checkpoint
from ftw_planet.evaluation import DENSE_LABEL_COUNTRIES, FULLDATA_REGIONS, macro_average
...
def evaluate(
    ckpt: Path, split: str, watershed: bool, tta: bool, backend: str, out_dir: Path
) -> dict[str, float]:
    countries = SPLITS[split]
    csv = out_dir / "metrics.csv"
    evaluate_checkpoint(
        ckpt, csv, countries, tta=tta, watershed=watershed, min_pad_size=512, backend=backend
    )
    return macro_average(csv, countries)
...
def cmd_eval(args: argparse.Namespace) -> int:
    ...
    summary = evaluate(ckpt, args.split, not args.no_ws, not args.no_tta, args.backend, out_dir)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--split", choices=SPLITS, default="dense10")
    p.add_argument("--backend", choices=["planet", "s2"], default="planet")
    ...
    p.add_argument("--out", help="output directory for the metrics CSV")
