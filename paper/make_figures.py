...
m = base[["country", "pixel_level_iou"]].rename(columns={"pixel_level_iou": "base"}).merge(
...
ax.set_yticks(y); ax.set_yticklabels(m["country"])
...
# --- Table snippet ----------------------------------------------------------
FULL = {"austria", "belgium", "cambodia", "corsica", "croatia", "denmark",
        "estonia", "finland", "france", "germany", "latvia", "lithuania",
        "luxembourg", "netherlands", "slovakia", "slovenia", "south_africa",
...
print(f"baseline (PRUE recipe) iou: {base_pp['pixel_level_iou']:.3f}  objF1_ws: {base_pp['object_ws_f1']:.3f}")
print(f"SDF model (crop512+SDF)    iou: {sdf_pp['pixel_level_iou']:.3f}  objF1_ws: {sdf_pp['object_ws_f1']:.3f}")
