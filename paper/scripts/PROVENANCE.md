...
| --------------------------- | ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tab:resolution_ablation`   | `hpc/resolution_ablation.sbatch` + `scripts/eval/macro_resolution_ablation.py`        | `logs/resolution_ablation_macro.csv`. Three conditions (planet3m / s2up / s2nat10) of `polygon_metrics_eval.py --gt-polygons-root` (+ `--score-gsd-m 10` for s2nat10), per-country over the dense-10, macro-averaged PQ by area bin. Real 3\,m vs upsampled-S2 vs native-10\,m S2, scored against true polygons. |
| `tab:representation_ceiling` | `scripts/eval/representation_ceiling.py` (model-free)                                     | `logs/representation_ceiling.csv` (POOLED rows). Rasterizes the true FTW polygons (`data/ftw_polygons_clipped`) at genuine 3\,m vs 10\,m UTM grids, 1-px boundary between touching parcels, vectorizes, IoU$>$0.5 matches back to truth; recovery recall by GT area bin, pooled over the 10 dense held-out regions. No model/checkpoint involved. |
...
| `fig:per_country_bars`      | `per_country_pq_objf1.py`                                                                 | three split panels: (1) per-region $\Delta$PQ from `logs/polygon_metrics/planet_b3_augmax_full_22.csv` (Planet, all-23-region run; the 37.9 macro) vs `logs/polygon_metrics/s2_upsampled_b7_augmax_full_22.csv` (S2-B7); (2) FTW-official per-region Obj-F1 (`logs/fulldata_eval/`, `logs/ftw_official/b7_*.csv`; Brazil n/a); (3) per-region $\Delta$PQ on $<0.5$\,ha GT fields from `logs/area_bins_per_country/{planet_b3,s2_b7}_full23_small.csv` (same protocol as panel 1; `hpc/area_bins_per_country.sbatch`, watershed+TTA, min-pad 512, area bins 0.5,2 ha, Planet 3\,m / S2 upsample-512 5\,m). If logs are absent, the script falls back to embedded plotted deltas matching the committed figure. |
